...
if ($Serial -notlike "emulator-*") {
    throw "Stage 16 visual QA only permits emulator-* serials. Refusing '$Serial'."
}
...
$remoteDirectory = "/sdcard/Android/data/$targetPackage/files/stage16-visuals"
$outputDirectory = Join-Path $repoRoot "build\reports\stage16-visuals"

& $runner `
    -Serial $Serial `
    -TestClass "com.qixuan.channelvideoflow.visual.Stage16VisualSnapshotTest" `
    -SkipBuild:$SkipBuild
if ($LASTEXITCODE -ne 0) {
    throw "Stage 16 visual instrumentation failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Could not pull Stage 16 screenshots from '$remoteDirectory'."
}

$screenshots = @(Get-ChildItem -LiteralPath $outputDirectory -Filter "*.png" -File)
if ($screenshots.Count -lt 7) {
    throw "Expected at least 7 Stage 16 screenshots, found $($screenshots.Count)."
}
...
Write-Output "STAGE16_VISUAL_QA_RESULT=PASS"
